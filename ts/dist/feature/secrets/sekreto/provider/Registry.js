...
function register(def) {
    if (null == def || 'string' !== typeof def.name || '' === def.name) {
        throw new support_1.SekretoError('sekreto: invalid provider definition');
    }
    registry[def.name] = def;
}
function registered(kind) {
...
function makeprovider(spec) {
    ...
    if (null == def) {
        throw new support_1.SekretoError('sekreto: unknown provider kind: ' + String(kind) +
