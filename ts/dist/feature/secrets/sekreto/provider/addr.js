...
function checkaddr(addr) {
    ...
    if (!addr.startsWith('http://')) {
        throw new support_1.SekretoError('sekreto: not an http(s) address: ' + addr);
    }
    ...
    try {
        ...
        throw new support_1.SekretoError('sekreto: not a valid http(s) address: ' + addr);
    }
    if ('localhost' === host || '127.0.0.1' === host || '::1' === host || '[::1]' === host) {
        return;
    }
    throw new support_1.SekretoError('sekreto: refusing to send a token in plaintext to ' + addr + ' (use https)');
}
