...
const TEST_SEED = { "entity": { "element": { "test01": { "id": "test01" } }, "group": { "test01": { "id": "test01" } }, "isotope": { "test01": { "id": "test01" } }, "series": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+ElementdemoSDK\b/.test(code) ||
        /\bElementdemoSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+ElementdemoSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/ElementdemoSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
