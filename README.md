...
> TypeScript, Python, Golang, Ruby, Bash, Java SDKs — all generated from one OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
...
client = ElementdemoSDK.test({
  "entity" => { "isotope" => { "test01" => { "id" => "test01" } } },
})
isotopes = client.Isotope.list()
```
...
| Ruby | `voxgig-sdk-elementdemo` | publish pending — [install from git tag](https://github.com/voxgig-sdk/voxgig-elementdemo-sdk/releases) |
...
| Surface | Path |
| --- | --- |
| **SDK** (TypeScript, Python, Golang, Ruby, Bash, Java) | `ts/` `py/` `go/` `rb/` `bash/` `java/` |
...
```ruby
require_relative "Elementdemo_sdk"

client = ElementdemoSDK.new({
  "apikey" => ENV["ELEMENTDEMO_APIKEY"],
})

# List all elements (returns an Array; raises on error)
elements = client.Element.list
puts elements

# Load a specific element (returns the ENTITY; call data_get for the record)
element = client.Element.load({ "id" => "example_id" })
puts element
```
