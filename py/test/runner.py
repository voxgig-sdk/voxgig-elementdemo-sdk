...
class ElementdemoTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ElementdemoTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ElementdemoTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ElementdemoTestRunner.getenv("ELEMENTDEMO_TEST_LIVE")
        override = ElementdemoTestRunner.getenv("ELEMENTDEMO_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ElementdemoTestRunner.getenv(key)
        ...
        explain = ElementdemoTestRunner.getenv("ELEMENTDEMO_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ELEMENTDEMO_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ElementdemoTestRunner._test_control is not None:
            return ElementdemoTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ElementdemoTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ElementdemoTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ElementdemoTestRunner.load_test_control()
...
def load_env_local():
    ElementdemoTestRunner.load_env_local()


def env_override(m):
    return ElementdemoTestRunner.env_override(m)


def entity_data(v):
    return ElementdemoTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ElementdemoTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ElementdemoTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ElementdemoTestRunner.load_test_control()


def live_client_options():
    return ElementdemoTestRunner.live_client_options()


def live_delay_ms():
    return ElementdemoTestRunner.live_delay_ms()
