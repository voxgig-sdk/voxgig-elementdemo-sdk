...
from elementdemo_sdk.utility import register

# Load features
from elementdemo_sdk.feature.base_feature import ElementdemoBaseFeature
from elementdemo_sdk.features import _has_feature, _make_feature
...
class ElementdemoSDK:
    ...
    def get_utility(self):
        return ElementdemoUtility.copy(self._utility)
    ...
    def _op_denied(self, op):
        ...
        return {
            "ok": False,
            "err": Exception(
                "ElementdemoSDK: " + op + ": operation not allowed by"
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("ElementdemoSDK: graphql: " + str(msg))
            res["graphql"] = errors
    ...
    def Element(self, data=None) -> "ElementEntity":
        ...
        from elementdemo_sdk.entity.element_entity import ElementEntity
        return ElementEntity(self, data)


    def Group(self, data=None) -> "GroupEntity":
        """Entity factory: client.Group().list() / client.Group().load({"id": ...})."""
        from elementdemo_sdk.entity.group_entity import GroupEntity
        return GroupEntity(self, data)


    def Isotope(self, data=None) -> "IsotopeEntity":
        """Entity factory: client.Isotope().list() / client.Isotope().load({"id": ...})."""
        from elementdemo_sdk.entity.isotope_entity import IsotopeEntity
        return IsotopeEntity(self, data)


    def Series(self, data=None) -> "SeriesEntity":
        """Entity factory: client.Series().list() / client.Series().load({"id": ...})."""
        from elementdemo_sdk.entity.series_entity import SeriesEntity
        return SeriesEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "ElementdemoSDK":
        if sdkopts is None:
            sdkopts = {}
