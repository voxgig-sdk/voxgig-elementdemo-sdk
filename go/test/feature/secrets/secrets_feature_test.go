...
import (
	...
	feat "github.com/voxgig-sdk/voxgig-elementdemo-sdk/go/feature"
)

const envprefix = "ELEMENTDEMO_TEST_SECRETS_"
...
func secretsClient(w *wire, sdkopts map[string]any) *sdk.ElementdemoSDK {
	opts := map[string]any{
	...
	return withSecrets(func(extend bool) *sdk.ElementdemoSDK {
		if extend {
			opts["extend"] = []any{feat.NewSecretsFeature()}
		}
		return sdk.NewElementdemoSDK(opts)
	})
}
...
func withSecrets(build func(extend bool) *sdk.ElementdemoSDK) *sdk.ElementdemoSDK {
	client := build(false)
...
func secretsFeatureOf(client *sdk.ElementdemoSDK) *feat.SecretsFeature {
...
func driveEntityOpUntil(t *testing.T, client *sdk.ElementdemoSDK, what string, stop func() bool) {
	t.Helper()
...
func driveEntityOp(t *testing.T, client *sdk.ElementdemoSDK, w *wire) {
	t.Helper()
	before := len(w.api())
...
func TestSecretsExchange(t *testing.T) {
	...
	t.Run("test mode buys nothing and needs no token endpoint", func(t *testing.T) {
		...
		w := makewire()
		client := withSecrets(func(extend bool) *sdk.ElementdemoSDK {
			opts := map[string]any{
				"system": map[string]any{"fetch": w.fetch},
				"feature": secretsOpts(map[string]any{
					"name":     "refresh_token",
