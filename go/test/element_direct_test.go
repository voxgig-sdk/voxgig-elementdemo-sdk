...
func TestElementDirect(t *testing.T) {
	t.Run("direct-list-element", func(t *testing.T) {
		setup := elementDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-element", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-element", func(t *testing.T) {
		setup := elementDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-element", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type elementDirectSetupResult struct {
...
func elementDirectSetup(mockres any) *elementDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["ELEMENTDEMO_TEST_ELEMENT_ENTID"]; ok {
		...
		return &elementDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &elementDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
