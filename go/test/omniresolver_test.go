...
import (
	...
	omni "github.com/voxgig-sdk/voxgig-elementdemo-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ElementdemoError); is {
				out := map[string]any{
					"name":    "ElementdemoError",
...
func omniCtx(arg any, client *sdk.ElementdemoSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ElementdemoSDK); is {
			client = live
