# Elementdemo SDK — elementcard feature (bash).
#
...
#
#   ELEMENTDEMO_ELEMENTCARD_ACTIVE=1   switch on
#   ELEMENTDEMO_ELEMENTCARD_PRINT=1    also print each card (stderr, so
#                                     stdout stays clean JSON)
#
# State mirrors the other targets' client._elementcard record:
# ELEMENTDEMO_ELEMENTCARD_COUNT and ELEMENTDEMO_ELEMENTCARD_LAST.

_elementdemo_register_feature "elementcard"

ELEMENTDEMO_ELEMENTCARD_COUNT=0
ELEMENTDEMO_ELEMENTCARD_LAST=""
...
elementdemo_elementcard_render() {
  local rec="${1:-$(cat)}"
  jq -r '
...
_elementdemo_elementcard_shaped() {
  jq -e '
    type == "object"
...
elementdemo_feature_elementcard_PreResult() {
  [ -n "${RESULT_BODY:-}" ] || return 0
  _elementdemo_elementcard_shaped "$RESULT_BODY" || return 0

  ELEMENTDEMO_ELEMENTCARD_LAST=$(elementdemo_elementcard_render "$RESULT_BODY")
  ELEMENTDEMO_ELEMENTCARD_COUNT=$((ELEMENTDEMO_ELEMENTCARD_COUNT + 1))

  if [ "1" = "${ELEMENTDEMO_ELEMENTCARD_PRINT:-}" ] \
    || [ "true" = "${ELEMENTDEMO_ELEMENTCARD_PRINT:-}" ]; then
    printf '%s\n' "$ELEMENTDEMO_ELEMENTCARD_LAST" >&2
  fi
}
