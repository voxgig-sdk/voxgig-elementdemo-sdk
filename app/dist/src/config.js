...
export function accountList(env = process.env) {
    ...
    return raw
        ...
        if (0 >= at) {
            throw new Error(`ACCOUNTS entry '${pair}' is not '<id>:<refresh-token>'`);
        }
        return {
            id: pair.slice(0, at),
...
export function accessTokenUses(env = process.env) {
    ...
    if (!Number.isFinite(n) || 1 > n) {
        throw new Error(`ACCESS_TOKEN_USES must be a positive integer, got '${raw}'`);
    }
    return n;
}
...
export function debugRouteEnabled(env = process.env) {
