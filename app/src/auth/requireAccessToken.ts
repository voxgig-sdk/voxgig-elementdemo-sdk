...
export function bearerToken(request: FastifyRequest): string {
  ...
  if (null == header || '' === header.trim()) {
    throw new AuthError(
      'missing Authorization header: expected `Authorization: Bearer <access-token>`')
  }
  ...
  if (null == match) {
    throw new AuthError(
      'malformed Authorization header: expected `Authorization: Bearer <access-token>`')
  }
...
export async function requireAccessToken(request: FastifyRequest): Promise<void> {
  const accountId = (request.params as { account_id?: string }).account_id
