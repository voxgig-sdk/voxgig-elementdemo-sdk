...
export const isotopeHandlers = {
  async list(
    request: FastifyRequest<{ Params: { element_id: string } }>,
    reply: FastifyReply
  ) {
  ...
  async get(
    request: FastifyRequest<{
      Params: { element_id: string; isotope_id: string }
    }>,
    reply: FastifyReply
  ) {
    const isotopeStore = request.server.isotopeStore
  ...
  async create(
    request: FastifyRequest<{ Params: { element_id: string }; Body: CreateIsotopeInput }>,
    reply: FastifyReply
  ) {
    const elementStore = request.server.elementStore
    const isotopeStore = request.server.isotopeStore
    ...
    if (request.body.element_id !== request.params.element_id) {
      throw new ValidationError(
        'element_id in body must match element_id in path'
      )
    }
  ...
  async update(
    ...
    if (
      undefined !== request.body.element_id &&
